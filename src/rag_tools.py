...
def _simple_embedding(texts: List[str], dim: int = 512) -> np.ndarray:
    # Deterministic bag-of-hash tokens fallback embedding
...
def get_embeddings(texts: List[str], model: str = "openai") -> np.ndarray:
    # If OPENAI_API_KEY is set and openai is installed, use it. Otherwise fallback.
    if os.getenv("OPENAI_API_KEY") and openai is not None:
        # Use OpenAI embeddings (batching naive)
...
def build_faiss_index(vectors: np.ndarray):
    ...
    # For IP we should normalize vectors
...
def get_weather(fecha: str) -> dict:
    try:
        ...
        raise
    # Simulated weather: deterministic from hash
